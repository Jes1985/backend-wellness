...
class StripeController {
  ...
  initializeRoutes() {
    this.router.get(
      `${this.path}/get_account_balance/`,
      verifyUser,
      this.stripeService.getAccountBalance
    );

    this.router.get(
      `${this.path}/getcarte_info/`,
      verifyUser,
      this.stripeService.getCarteInfo
    );

    this.router.get(
      `${this.path}/plan/manage_plan/`,
      verifyUser,
      this.stripeService.getManagePlan
    );

    this.router.get(
      `${this.path}/plan/sublist/`,
      verifyUser,
      this.stripeService.getSublistPlan
    );

    this.router.get(
      `${this.path}/plan/subscription/`,
      verifyUser,
      this.stripeService.getSubscriptionPlan
    );

    this.router.get(
      `${this.path}/plan/user_subscription/`,
      verifyUser,
      this.stripeService.getUserSubscriptionPlan
    );

    this.router.get(
      `${this.path}/update_carte_info/`,
      verifyUser,
      this.stripeService.getUpdateCarteInfo
    );
